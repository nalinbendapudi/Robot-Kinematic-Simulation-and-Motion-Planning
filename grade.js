//CI grader generates report at 03-12-2020-19-20-08
grade = 
{
  "bnalin": {
    "name": "Bendapudi, Nalin", 
    "unique_name": "bnalin", 
    "email": "bnalin@umich.edu", 
    "course": "ROB 511 F20", 
    "repository": "git@github.com:nalinbendapudi/Autonomous-Robotics.git", 
    "assignments": {
      "Honor": {
        "status": "CHECK", 
        "comments": [
          ""
        ]
      }, 
      "PathPlan_Heap": {
        "status": "CHECK", 
        "test results": {
          "Test 8": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 9": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 6": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 7": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 5": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }
        }, 
        "comments": [
          "Point: 4/4, due day check pass, good work!"
        ]
      }, 
      "PathPlan_AStar": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }
        }, 
        "comments": [
          "Point: 8/8, due day check pass, good work!"
        ]
      }, 
      "PathPlan_DFS": {
        "status": "CHECK", 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "PathPlan_BFS": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "PathPlan_BestFirst": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": "N/A"
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "Pendularm_Euler": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 4/4, due day check pass, good work!"
        ]
      }, 
      "Pendularm_VelocityVerlet": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 4/4, due day check pass, good work!"
        ]
      }, 
      "Pendularm_PID": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 4/4, due day check pass, good work!"
        ]
      }, 
      "Pendularm_Verlet": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 1/1, due day check pass, good work!"
        ]
      }, 
      "Pendularm_RK4": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "Pendularm_Double": {
        "status": "CHECK", 
        "comments": [
          "Point: 3/3, due day check pass, good work!"
        ]
      }, 
      "FK_MatrixRoutines": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FK_Transforms": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 6": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 7": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 5": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 8/8, due day check pass, good work!"
        ]
      }, 
      "FK_JointRendering": {
        "status": "CHECK", 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FK_BaseOffset": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 6": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 7": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 4": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 5": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FK_NewRobot": {
        "status": "CHECK", 
        "comments": [
          "Point: 4/4, wow, really nice robot! Good job!"
        ]
      }, 
      "FSMDance_Quaternion": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 6/6, due day check pass, good work!"
        ]
      }, 
      "FSMDance_BaseControl": {
        "status": "CHECK", 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FSMDance_SetpointControl": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FSMDance_FSM": {
        "status": "CHECK", 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FSMDance_JointLimits": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FSMDance_Prismatic": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 2/2, due day check pass, good work!"
        ]
      }, 
      "FSMDance_Rosbridge": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }, 
      "IK_Jacobian": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 6/6, due day check pass, good work!"
        ]
      }, 
      "IK_JTranspose": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 3/3, due day check pass, good work!"
        ]
      }, 
      "IK_JPseudoInverse": {
        "status": "CHECK", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 3/3, due day check pass, good work!"
        ]
      }, 
      "IK_EulerAngles": {
        "status": "CHECK", 
        "test results": {
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "Point: 6/6, due day check pass, good work!"
        ]
      }, 
      "MotionPlan_Collision": {
        "status": "PASS", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "3 out of 3 test cases passed"
        ]
      }, 
      "MotionPlan_2DRRTConnect": {
        "status": "PASS", 
        "test results": {
          "Test 2": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 3": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 0": {
            "Status": "Succeed", 
            "Stderr": ""
          }, 
          "Test 1": {
            "Status": "Succeed", 
            "Stderr": ""
          }
        }, 
        "comments": [
          "4 out of 4 test cases passed"
        ]
      }, 
      "MotionPlan_CSpaceRRTConnect": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }, 
      "MotionPlan_RRTStar": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }, 
      "Pitch": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }, 
      "Pitch_Advanced": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }, 
      "Advanced": {
        "status": "PENDING", 
        "comments": [
          ""
        ]
      }
    }, 
    "quizzes": [
      {
        "status": 4.0
      }, 
      {
        "status": 4.0
      }, 
      {
        "status": 4.0
      }, 
      {
        "status": 4.0
      }, 
      {
        "status": "PENDING"
      }
    ], 
    "participation": {
      "status": 1.0, 
      "comments": "Pod submitted name and logo\nPod submitted dance off video "
    }
  }
}